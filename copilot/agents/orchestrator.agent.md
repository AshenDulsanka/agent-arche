...
1. `.github/skills/caveman/SKILL.md` — load and apply before first response, active all session
2. `.github/skills/project-startup/SKILL.md` — first-time project setup only (see Startup Flow)
...
- **If marker absent**: run the `caveman` compression subflow (`steps/compress.md`) on `.github/copilot-instructions.md` and any verbose files in `.github/memory/`. The subflow overwrites with compressed version and saves `.original.md` backup. After compression, add `<!-- caveman compressed -->` to the top of each compressed file.
- **If marker present**: skip compression entirely.

### Step 0B: Project startup (first run only)
Check if `.github/memory/_MOC.md` exists and has content, and whether `docs/agents/issue-tracker.md`, `docs/agents/triage-labels.md`, and `docs/agents/domain.md` exist.
...
| Task | Skills |
|------|--------|
| Product brainstorming | `product-brainstorming` |
| PRD | `create-prd` |
| Epics and stories | `create-epics-and-stories` |
| Pre-planning interrogation | `grill-me` for projects not started, `grill-with-docs` for existing projects |
| Project startup / issue tracker labels | `project-startup`, `triage` |
| Bug diagnosis | `diagnose` |
| Prototype | `prototype` |
| Review | `review` |
| Architecture improvement | `improve-codebase-architecture` |
| Handoff | `handoff` |
| New UI / visual direction / design system | `design` (`direction` + `implementation`) |
| Landing/dashboard/app design | `design` (`direction` + `implementation`) |
| Premium or polished UI | `design` with optional `styles/soft.md` / `styles/minimalist.md` / `styles/brutalist.md` |
| UI quality | `design` (`steps/ui-audit.md` / `steps/critique.md`) |
| Visual/layout fix | `design` (`steps/redesign.md` / `steps/animate.md`) |
| Cinematic scroll | `design` (`steps/gsap.md`) |
| UI performance | `design` (`steps/ui-optimize.md`) |
| Aesthetic | `design` (`styles/soft.md` / `styles/minimalist.md` / `styles/brutalist.md`) |
...
| SEO | `seo` |
| Git/PR | `git` |
| Project startup | `project-startup` (first run only) |
| Compression | `caveman` (`steps/compress.md`, first run only) |
| TDD | `tdd` |
...
- New feature (full) → `Planner (product brainstorming/PRD/epics-stories as needed) → Researcher → Coder + Designer → Code-reviewer + Security-auditor + UX-reviewer → Tester → Docs-updater` (6 phases)
...
|---|---|
| New feature (full) | Planner (product brainstorming/PRD/epics-stories as needed) → Researcher → Coder + Designer (parallel if independent) → Code-reviewer + Security-auditor + UX-reviewer (parallel) → Tester → Docs-updater |
