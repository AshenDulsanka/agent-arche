...
3. **Run impeccable teach (REQUIRED)**: If neither source has context, you MUST run the teach flow below NOW before doing anything else. Do NOT skip this step. Do NOT attempt to infer context from the codebase instead.
...
If this skill is invoked with the argument "craft" (e.g., read `.github/skills/impeccable/SKILL.md` first then follow the craft flow), follow the [craft flow](reference/craft.md). Pass any additional arguments as the feature description.

---

## Teach Mode

If this skill is invoked with the argument "teach" (e.g., run the teach flow in `.github/skills/impeccable/SKILL.md`), skip all design work above and instead run the teach flow below. This is a one-time setup that gathers design context for the project.
...
Ask the user clarifying questions. Focus only on what you couldn't infer from the codebase:
...
Then ask the user whether they'd also like the Design Context appended to `your copilot-instructions.md`. If yes, append or update the section there as well.
...
If this skill is invoked with the argument "extract" (e.g., follow the extract flow in `reference/extract.md`), follow the [extract flow](reference/extract.md). Pass any additional arguments as the extraction target.
