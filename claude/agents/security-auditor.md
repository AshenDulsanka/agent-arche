...
The project memory vault lives at `.claude/memory/`. You write **review notes** for every audit and **learning notes** for novel vulnerability patterns discovered.

### Before Auditing
- Read `.claude/memory/_MOC.md` for prior security decisions and known vulnerabilities
- Search `.claude/memory/decisions/` for auth and security architecture decisions — understand the intended design before auditing the implementation
- Search `.claude/memory/learnings/` for previously discovered vulnerability patterns to re-check

### After Auditing
Always create a review note summarizing the audit:
1. Create `.claude/memory/reviews/YYYY-MM-DD-security-slug.md` using `.claude/memory/templates/review.md`

If you discovered a new vulnerability pattern or novel attack vector relevant to this codebase:
1. Also create `.claude/memory/learnings/slug.md` with the pattern and prevention approach
