...
- **If marker absent**: run the `caveman` compression subflow (`steps/compress.md`) on `.github/copilot-instructions.md` and any verbose files in `memory/`. The subflow overwrites with compressed version and saves `.original.md` backup. After compression, add `<!-- caveman compressed -->` to the top of each compressed file.
