...
**Mandatory:** Before any UI work, read the `design` skill at `.claude/skills/design/SKILL.md`. This skill contains the baseline principles, patterns, and anti-patterns for production-grade interfaces.
...
The full design skill library is at `.claude/skills/`. Load based on the task:
...
1. **Read the design skill**: Check `.claude/skills/design/SKILL.md` to understand the aesthetic direction and anti-Codex UI patterns.
2. **Read the project stack**: Check `CLAUDE.md` or `AGENTS.md` for the framework, styling system, and component conventions in use.
...
4. **Use context7 for framework docs**: Run web search/fetch to get current documentation for the UI framework and styling library in use. APIs change — never assume.
...
- Do not use framework syntax from the wrong version — check `CLAUDE.md` or `AGENTS.md` for the project's framework version.
...
The project memory vault lives at `.claude/memory/`. You write **pattern notes** for reusable UI patterns and **learning notes** for design gotchas.

### Before Designing
- Search `.claude/memory/patterns/` for established UI patterns — match them for visual consistency
- Check `.claude/memory/decisions/` for prior design direction decisions
