...
1. `.claude/skills/caveman/SKILL.md` — load before first response, active all session
2. `.claude/skills/analyze-codebase/SKILL.md` — first-time project setup only (see Startup Flow)
3. `.claude/skills/caveman-compress/SKILL.md` — first run only (see Startup Flow)
...
- **If marker absent**: run `caveman-compress` skill on `.github/copilot-instructions.md` and any verbose files in `.claude/memory/`. Skill overwrites with compressed version and saves `.original.md` backup. After compression, add `<!-- caveman compressed -->` to the top of each compressed file.
- **If marker present**: skip compression entirely.

### Step 0B: Analyze codebase (first run only)
Check if `.claude/memory/_MOC.md` exists and has content.
- **If empty or missing**: invoke **Researcher** or run `analyze-codebase` skill to create the full project memory structure in `.claude/memory/`.
- **If populated**: skip.
