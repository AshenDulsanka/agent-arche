...
Use web search/fetch to get current, accurate documentation for any framework, library, or API involved. Never rely solely on training data — especially for:
...
The project memory vault lives at `.claude/memory/`. You write **learning notes** for key findings and **decision notes** when recommending one approach over another.

### Before Researching
- Read `.claude/memory/_MOC.md` to see what has already been researched
- Search `.claude/memory/learnings/` to avoid re-documenting known topics
