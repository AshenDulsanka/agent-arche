...
model: sonnet
tools: Read, Grep, Glob, WebSearch, TodoRead, TodoWrite, Task
---

<!-- Note: Memory requires the .claude/memory/ vault to be seeded via the analyze-codebase skill. -->
...
Read `.claude/skills/architecture/SKILL.md` if you need to understand the project structure. Read `.claude/skills/coding-standards/SKILL.md` to understand project conventions before delegating implementation.

**Non-negotiable constraints for all agents**:
- All user input must be validated at API boundaries before use
- Read and follow project conventions in `CLAUDE.md` or `AGENTS.md` for project overview, tech stack, and key conventions
...
Specialist agents load skills from `.claude/skills/` — delegate with skill context when relevant:
...
Classify the request (see Step 1 table below), then immediately call asking questions with a single question:
...
The project memory vault lives at `.claude/memory/`. Open this folder in Obsidian to explore the full knowledge graph. You are responsible for creating and closing the session note for every pipeline run.

### At Pipeline Start — Before Phase 1
0. **Compress context files** (first session, or whenever files feel verbose) — run the `caveman-compress` skill on `CLAUDE.md` or `AGENTS.md` and any bloated files in `.claude/memory/`. Overwrites with compressed version, saves backup as `.original.md`. Reduces input tokens on every future read.
1. Read `.claude/memory/_MOC.md` to load prior context (decisions, patterns, learnings)
2. Create a session note at `.claude/memory/sessions/YYYY-MM-DD-task-slug.md` using `.claude/memory/templates/session.md`
...
2. Append to `.claude/memory/_MOC.md`:
